...
bool prevent_updates_to_viz = false;
...
void printVoices();
void setVolume() {
  sgtl5000_1.volume(0.8 * (analogRead(A16) - 1) / 1022.0);
}
...
void OnNoteOn(byte channel, byte note, byte velocity) {
  ...
  if (velocity == 0) {
    ...
    vizNoteOff(channel, note, velocity);
  } else {
    ...
    vizNoteOn(channel, note, velocity);
  }
}

void allNotesOff() {
  prevent_updates_to_viz = true;
  for (int i = 0 ; i < TOTAL_VOICES; i++) {
    wavetable[i].stop();
  }
}
